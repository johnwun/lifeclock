...
class App extends Component {
  constructor(props) {
    ...
    const defaultKeys = keys(omitBy(data, e => !e.default));

    const generateOptions = dataKeys =>
      dataKeys.sort().filter(e => e !== "cfg");
    //.map(key => ({ key: key, id: key, label: key }));

    const graphMin = new Date(get(data, "cfg.start")).getTime();
    const graphMax = new Date(get(data, "cfg.end")).getTime();
  ...
  handleChange({ currentTarget }) {
    this.setState({
      selectedItems: Array.from(currentTarget)
        .filter(e => e.selected)
        .map(e => e.value)
    });
  }
